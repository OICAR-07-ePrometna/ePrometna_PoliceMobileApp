...
describe('scanQRCodeAndGetData', () => {
  ...
  describe('successful QR code scanning', () => {
    it('should scan QR code and return complete data', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith('PUT response:', mockTempData);
      expect(mockConsoleLog).toHaveBeenCalledWith(
        'Temp data retrieved and deleted. Vehicle UUID:',
        'vehicle-uuid-456',
        'Driver UUID:',
        'driver-uuid-789'
      );
      expect(mockConsoleLog).toHaveBeenCalledWith('Vehicle response status:', 200);
      expect(mockConsoleLog).toHaveBeenCalledWith('Driver response status:', 200);
    });
    ...
    it('should handle different HTTP status codes', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith('Vehicle response status:', 201);
      expect(mockConsoleLog).toHaveBeenCalledWith('Driver response status:', 201);
    });
  });
  ...
  describe('validation errors', () => {
    it('should throw error when vehicleUuid is missing', async () => {
      ...
      expect(mockConsoleError).toHaveBeenCalledWith('Missing UUIDs in temp data response:', invalidTempData);
  ...
  describe('HTTP error handling', () => {
    it('should handle 404 error (QR code not found or already used)', async () => {
      ...
      expect(mockConsoleError).toHaveBeenCalledWith('Error scanning QR code and retrieving data:', {
        message: 'Not Found',
        status: 404,
        statusText: 'Not Found',
        data: { message: 'Temp data not found' },
        url: `/tempdata/${mockTempDataUuid}`,
        uuid: mockTempDataUuid,
      });
    });
